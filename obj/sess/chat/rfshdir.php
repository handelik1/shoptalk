<?php
 ...
 /*************************************************************************************
  This is the chat directory refresh routine. A registered user has the Chat section
  open, and a periodic request has been issued to refresh the listings of chats.
 ...
 debug_log_inputs('chat-rfshdir', $_POST);

 if(strcmp($send_request, 'whoozTalkinWutsssupWhat') == 0)		#  Run only if request came from JS
   {
     ...
     $active = $shoptalk_chatdir->retrieve_active_chats($kp, $link, false);
     $archived = $shoptalk_chatdir->retrieve_archived_chats($kp, $link, false);

     $outputstring  = 'ok|';									#  Build the output string
     $outputstring .= strlen($active).'|';
     $outputstring .= $active;
     $outputstring .= $archived;
 ...
 debug_log_outputs('chat-rfshdir', $outputstring);
