<?php
 ...
 /*************************************************************************************
  This is the chat landing page routine. A registered user has clicked "Chat" on the left-
  hand nav bar, meaning that they wish to take part in a chat.
  Either they will search for an existing chat and join it, or they will create a new
  seesion of their own. The default view is a roster of all existing chat sessions.
  We check the given credentials against existing records and either setup the page or
 ...
 debug_log_inputs('chat-home', $_POST);

 if(strcmp($send_request, 'letZtalkyYynot') == 0)				#  Run only if request came from JS
   {
     ...
     $shoptalk_chatdir = new ChatDirectory();
     $shoptalk_chattoolbar = new ChatToolbar();
     ...
     $outputstring .=       $shoptalk_chatdir->draw($kp, $link);#  Render the Chat Directory
     $outputstring .=       $shoptalk_chattoolbar->draw();		#  Render the Chat Toolbar
     $outputstring .=     '</div>';
     $outputstring .=   '</div>';
     $outputstring .= '</div>';
 ...
 debug_log_outputs('chat-home', $outputstring);
